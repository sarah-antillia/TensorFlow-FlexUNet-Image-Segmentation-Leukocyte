...
def split(master_dir, output_dir):
    categories = os.listdir(master_dir)
    random.seed(137)

    for category in categories:
      
      cat_dir = os.path.join(master_dir, category)
      images_dir = cat_dir + "/images/"
      masks_dir  = cat_dir + "/masks/"
      ...
      valid_dir   = os.path.join(output_dir, "valid/" + category) 
...
def copy(image_files, masks_dir, dataset_dir):
  ...
  for image_file in image_files:
    ...
    mask_filepath = os.path.join(masks_dir, mask_name)
...
if __name__ == "__main__":
  try:
    master_dir = "./Augmented-Leukocyte-LISC-PNG-master/"

    output_dir = "./Augmented-LISC-Leukocyte-PNG-ImageMask-Dataset/"
    if os.path.exists(output_dir):
      shutil.rmtree(output_dir)
    if not os.path.exists(output_dir):
      os.makedirs(output_dir)

    split(master_dir, output_dir)

    """
    subdirs = os.listdir(master_dir)
    for subdir in subdirs:
      full_subdir   = os.path.join(master_dir)
      images_subdir = os.path.join(full_subdir, subdir + "/images")
      masks_subdir  = os.path.join(full_subdir, subdir + "/masks")
      output_subdir = os.path.join(output_dir, subdir)
      os.makedirs(output_subdir)
      split_master(images_subdir, masks_subdir, output_subdir)
    """
